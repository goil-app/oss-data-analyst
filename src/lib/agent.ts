...
- Resolving ids to names with a second query (e.g. businessId -> ClientDB.Business): pair every row with its name by the exact 24-hex id (for many rows: cp /tmp/mongodb_result.json /tmp/first.json before the second query, then join both files with python3). Never pair by order or position; if an id has no match, say "(nom desconegut)"
